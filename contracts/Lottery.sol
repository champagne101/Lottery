...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // State variables 
    // Instance of Bound token (collateral currency for lotto)
    IERC20 internal bound_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInBound;    // The amount of bound for prize money
        uint256 costPerTicket;      // Cost per ticket in $bound
        ...
        returns(uint256 lotteryId)
    {
        ...
        require(
            _prizePoolInBound != 0 && _costPerTicket != 0,
    ...
    function withdrawBound(uint256 _amount) external onlyOwner() {
        bound_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        // Transfers the required bound to this contract
        bound_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInBound = allLotteries_[_lotteryId].prizePoolInBound.sub(prizeAmount);
        // Transfering the user their winnings
        bound_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        bound_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in bound the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInBound.mul(perOfPool);
